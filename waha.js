...
app.post('/start-session', async (req, res) => {
    try {
        ...
        if (!sock.authState.creds.registered) {
            try {
                // ننتظر شوية عشان الاتصال يستقر
                await new Promise(resolve => setTimeout(resolve, 2000));
                
                const code = await sock.requestPairingCode(phone);
                console.log(`🔑 كود الاقتران للرقم ${phone}: ${code}`);
                ...
                if (!responded) {
                    ...
                    return res.json({
                        ...
                        status: 'pairing_code',
                        code: code,
                        message: `استخدم الكود: ${code} للربط من إعدادات واتساب > الأجهزة المرتبطة > الربط برقم الهاتف`
                    });
                }
            } catch (err) {
                ...
                if (!responded) {
                    responded = true;
                    return res.json({ success: false, error: 'فشل طلب كود الاقتران: ' + err.message });
                }
            }
        }
...
setInterval(() => {
    ...
    for (const phone in sessions) {
        const sessionDir = path.join('./sessions', sessions[phone].sessionId);
        if (fs.existsSync(sessionDir)) {
            console.log(`💾 تم حفظ جلسة ${phone}`);
        }
    }
}, 300000); // كل 5 دقايق
